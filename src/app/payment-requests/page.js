      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '24px' }}>
          ...
          <div style={{
            ...
            {bankSettings.bank_upi_id && bankSettings.bank_upi_id.trim() !== '' ? (
              <>
                <div style={{
                  background: 'white',
                  padding: '12px',
                  ...
                  <img
                    src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(`upi://pay?pa=${bankSettings.bank_upi_id.trim()}&pn=${bankSettings.bank_name || 'SkyRelief'}`)}`}
                    alt="Bank UPI QR Code"
                    style={{ width: '150px', height: '150px', display: 'block' }}
                  />
                </div>
                <div style={{ fontSize: '0.9rem', fontWeight: '800', color: '#38bdf8' }}>{bankSettings.bank_upi_id}</div>
                <div style={{ fontSize: '0.75rem', color: '#94a3b8', marginTop: '4px' }}>{bankSettings.bank_name || 'State Bank of India'}</div>
              </>
            ) : (
              <div style={{ padding: '30px 10px', color: '#94a3b8', fontSize: '0.85rem' }}>
                <div style={{ fontSize: '2rem', marginBottom: '8px' }}>📲</div>
                Add UPI ID to generate live QR Code preview
              </div>
            )}
          </div>
